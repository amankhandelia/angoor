...
- **Display minification**: `_fmt_display(value)` = `_minify(_fmt(value))` — Apple-style middle truncation (`start…end`) at `_MAX_CELL_WIDTH` (40). Long cells are shortened for the table only; `v` opens a `CellViewerScreen` modal (`ModalScreen` with a read-only `TextArea`) showing the **full** raw value. The modal **owns** every key press (`on_key` calls `event.stop()` so `j`/`k`/`h`/`l`/etc. never leak to the parent app's `on_key`): `j`/`k`/`↓`/`↑` scroll the viewer; `yw`/`yy`/`yc` copy the **opened** cell (`self.text`), so scrolling never perturbs the underlying DataTable cursor (TODO v-mode bug guard); `Esc` dismisses. The footer swaps to viewer hints while open and is restored on close.

- **Footer option bar**: a lazygit-style `key: description | key: description | …` line rendered as a Rich `Text` (bold-cyan keys, dim `|` separators) by `_render_hints(...)`. There are four variants: `_help_text()` (cell-mode), `_header_help_text()` (header-mode), `_search_help_text(mode)` ("ROW SEARCH"/"COL SEARCH" prefix), `_viewer_help_text()` (cell viewer open). The cell/header/search/viewer hint lists are module-level tuples (`_CELL_HINTS`, `_HEADER_HINTS`, `_SEARCH_HINTS`, `_VIEWER_HINTS`). `ViewerApp._update_footer()` selects the right one based on `_search_mode`/`_in_header`, and `CellViewerScreen.on_mount`/`on_unmount` swap the viewer variant in/out.

- **`ViewerDataTable`** subclasses `DataTable` so the cursor can be visually hosted on the header row. Heading state still lives in app-level `_in_header` (the base class can't hold a cursor on its header); the subclass adds `set_header_active_column(col: int | None)` which bumps `_update_count` (to invalidate the line/cell render caches — they are keyed on `_update_count`, not on `_header_active_column`) and refreshes. Its `_should_highlight` override returns True only for the header cell (`row == -1`) at the active column when in header mode, and **False for every data cell** when in header mode — so the parked row-0 cursor does NOT also paint a cursor on the first data row. `ViewerApp._sync_header_highlight()` is called after every navigation that may move the active column (entering/exiting header, `h`/`l`/`^`/`$`/`gg`/`G`, search landings) to keep the visual highlight in step with `_in_header` + the DataTable cursor column.
